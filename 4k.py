...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['11', '10', '6.0.1'])
            self.android_model = random.choice(['BRAVIA 4K'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {} VH2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.125 Safari/537.36'.format(self.android_version, self.android_model, self.build, self.browser_version))
